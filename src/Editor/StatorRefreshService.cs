...
namespace Stator.Editor
{
    public class StatorRefreshService
    {
        ...
        public void Refresh()
        {
            ...
            List<string> files = Generate(generator, factoryTypes);

            var allFiles = Directory.GetFiles(Settings.PathToGenerate, "*.cs")
                .Select(f => Path.Combine(Settings.PathToGenerate, f));
        ...
        private List<string> Generate(StatorCodeGenerator generator, Type[] factoryTypes)
        {
            ...
            foreach (var factoryType in factoryTypes)
            {
                ...
                var asmdefPath = AssetDatabase.FindAssets($"t:asmdef {factoryType.Assembly.GetName().Name}")
                    .Select(g => AssetDatabase.GUIDToAssetPath(g))
                    .Where(o => Path.GetFileName(o) == (factoryType.Assembly.GetName().Name + ".asmdef"))
                    .FirstOrDefault();
                ...
                if(asmdefPath != null)
                {
                    path = Path.Combine(Path.GetDirectoryName(asmdefPath), Settings.AutoGeneratedFolder,
